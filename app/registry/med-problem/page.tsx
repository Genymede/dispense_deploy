...
export default function MedProblemPage() {
  ...
  const openEdit = (row: any) => {
    ...
    setEditingId(row.problem_id); setResetKey(k => k + 1); setShowModal(true);
  };
  ...
  return (
    <MainLayout title="ปัญหาการใช้ยา" subtitle="Drug-Related Problems (DRP)">
      <DataTable cols={cols}
        ...
        searchPlaceholder="ค้นหายา, ผู้ป่วย, คำอธิบาย..."
        ...
        actionCol={row => (
          <RowActions
            ...
            onEdit={() => openEdit(row)}
            onDelete={async () => { await crudApi.deleteMedProblem(row.problem_id); setReload(r => r + 1); }}
          />
        )}
      ...
      <CrudModal open={showModal} onClose={() => setShowModal(false)}
        ...
        <FormGrid>
          ...
          <SearchSelect type="patient" label="ผู้ป่วย (ถ้ามี)"
